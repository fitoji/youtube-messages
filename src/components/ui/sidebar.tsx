...
function SidebarInset({
  ...
  ref,
  ...props
}: React.ComponentProps<"main"> & { ref?: React.Ref<HTMLElement> }) {
  return (
    <main
      ref={ref}
      className={cn(
...
function SidebarSeparator({
  ...
  ref,
  ...props
}: React.ComponentProps<typeof Separator> & { ref?: React.Ref<HTMLDivElement> }) {
  return (
    <Separator
      ref={ref}
...
function SidebarMenuSkeleton({
  ...
  // Deterministic width based on useId to avoid Math.random() in render
  const id = React.useId();
  const width = `${(id.charCodeAt(1) % 40) + 50}%`;
